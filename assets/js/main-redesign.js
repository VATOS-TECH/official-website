/* ============================================================
   VATOS — Main Page Redesign interactions
   GSAP + ScrollTrigger, IntersectionObserver, canvas particles.
...
(function () {
  ...
  if (REDUCE) {
    document.querySelectorAll('.rd-reveal, .rd-mask').forEach(function (el) { el.classList.add('is-in'); });
    var sub = document.getElementById('heroSub'); if (sub) sub.style.opacity = 1;
    return; // skip all scroll-driven motion
  }

  /* -------------------------------------------------- IntersectionObserver reveals */
  (function reveals() {
    var els = document.querySelectorAll('.rd-reveal, .rd-mask');
